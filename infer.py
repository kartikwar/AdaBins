...
class InferenceHelper:
	...
	def predict_dir(self, test_dir, out_dir):
		...
		for f in tqdm(all_files):
			image = np.asarray(Image.open(f).resize((640,480)), dtype='float32') / 255.
...
def convert_depth_to_rgb(depth_path):
	...
	im_rgb = Image.fromarray(im_array)
	return im_rgb
...
if __name__ == '__main__':
	...
	folder_path = '/home/ubuntu/kartik/AdaBins/val-dataset'
	...
	# test_dir = '/home/ubuntu/kartik/AdaBins/val-dataset/images'
	# out_dir = '/home/ubuntu/kartik/AdaBins/val-dataset/depth'
	#compute depth first
	inferHelper.predict_dir(images_folder, depth_folder)

	all_files = glob.glob(os.path.join(images_folder, "*"))

	for f_path in all_files:
		try:
			cv2.imwrite(f_path, cv2.resize(cv2.imread(f_path), (1000,1000)))
			basename = os.path.basename(f_path).split('.')[0]
			
			depth_path = os.path.join(depth_folder, basename + ".png")
			depth = convert_depth_to_rgb(depth_path)
			depth.resize((1000,1000))
			depth.save(depth_path)

			heatmap_path = os.path.join(heatmap_folder, basename + ".png")
			heatmap = convert_depth_map_to_color(depth_path)
			heatmap = cv2.resize(heatmap,(1000,1000))
	...
	# result = pred.squeeze()
	# result = (result * 1000).astype('uint16')
	# Image.fromarray(result).save("test_imgs/out.png")
	# import pdb
	# pdb.set_trace()
